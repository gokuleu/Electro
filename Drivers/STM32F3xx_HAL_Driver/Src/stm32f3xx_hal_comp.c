...
__weak void HAL_COMP_TriggerCallback(COMP_HandleTypeDef *hcomp)
{
  ...
  uint8_t temp;
  UNUSED(hcomp);
  uint32_t value = *(volatile uint32_t*)0x40010030;
static char complevel_prevstate=0;
  char complevel=((value>>30)&0x1);

  if (complevel==1)
  {
    hrtimer_disable();
    complevel_prevstate=1;
  }
  else{
    if (complevel_prevstate==1)
    {
      hrtimer_enable();
      complevel_prevstate=0;
    }
